 ...
 */
enum class P2TSHSpendType : uint8_t {
...
/**
 * Per-leaf data stored in P2TSHKeyMetadata.
 */
struct P2TSHLeafInfo {
    ...
    SERIALIZE_METHODS(P2TSHLeafInfo, obj) {
...
struct P2TSHKeyMetadata {
    ...
    P2TSHLeafInfo leaves[P2MR_NUM_LEAVES];       //!< Indexed by P2TSHSpendType
    ...
    const P2TSHLeafInfo& GetLeaf(P2TSHSpendType type) const {
...
private:
    ...
    std::map<uint256, P2TSHKeyMetadata> mapP2MRMetadata GUARDED_BY(cs_p2mr);

    //! Preferred spend type (which leaf to use when signing)
    P2TSHSpendType m_preferred_spend_type{P2TSHSpendType::SCHNORR};
...
public:
    ...
    const P2TSHKeyMetadata* GetP2MRMetadata(const uint256& merkle_root) const;
    ...
    bool SignP2MRInput(CMutableTransaction& tx, unsigned int input_idx,
                       ...
                       P2TSHSpendType spend_type) const EXCLUSIVE_LOCKS_REQUIRED(cs_p2mr);
    ...
    void SetPreferredSpendType(P2TSHSpendType type) { m_preferred_spend_type = type; }
    P2TSHSpendType GetPreferredSpendType() const { return m_preferred_spend_type; }
    ...
    bool WriteKey(WalletBatch& batch, const uint256& merkle_root,
                  const P2TSHKeyMetadata& metadata) EXCLUSIVE_LOCKS_REQUIRED(cs_p2mr);

    // ---- String conversion ----
    static std::string SpendTypeToString(P2TSHSpendType type);
    static std::optional<P2TSHSpendType> StringToSpendType(const std::string& str);
