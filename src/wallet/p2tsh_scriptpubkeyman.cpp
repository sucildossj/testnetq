...
CScript P2TSHScriptPubKeyMan::BuildSchnorrLeafScript(
...
CScript P2TSHScriptPubKeyMan::BuildSLHDSALeafScript(
...
CScript P2TSHScriptPubKeyMan::BuildHybridLeafScript(
    const std::vector<unsigned char>& schnorr_pk,
    const std::vector<unsigned char>& slh_dsa_pk)
{
    CScript script;
...
bool P2TSHScriptPubKeyMan::BuildP2MRTree(
    const std::vector<unsigned char>& schnorr_pk,
    const std::vector<unsigned char>& slh_dsa_pk,
    P2TSHKeyMetadata& metadata) const
{
    ...
    {
        P2TSHLeafInfo& info = metadata.leaves[static_cast<uint8_t>(P2TSHSpendType::SCHNORR)];
    ...
    {
        P2TSHLeafInfo& info = metadata.leaves[static_cast<uint8_t>(P2TSHSpendType::SLH_DSA)];
    ...
    {
        P2TSHLeafInfo& info = metadata.leaves[static_cast<uint8_t>(P2TSHSpendType::HYBRID)];
    ...
    WalletLogPrintf("P2TSH: Built 3-leaf tree (root: %s)\n", root_hash.GetHex());
    return true;
}
...
util::Result<CTxDestination> P2TSHScriptPubKeyMan::GetNewP2MRDestination()
{
    LOCK(cs_p2mr);
    ...
    if (m_pqhd && m_pqhd->IsSeeded()) {
        ...
        WalletLogPrintf("P2TSH: PQHD derived keys at m/360'/%u'/%u'/[340,205]'/0'/%u\n",
        ...
        WalletLogPrintf("P2TSH: Generated random keys (non-HD)\n");
    }
    ...
    WitnessV2P2TSH p2mr_dest(metadata.merkle_root);
    ...
    WalletLogPrintf("P2TSH: New address (root: %s) with 3 spend paths\n",
...
bool P2TSHScriptPubKeyMan::SetSeed(
    ...
    uint32_t coin_type, uint32_t account)
{
    ...
    WalletLogPrintf("P2TSH: PQHD initialized (coin_type=%u, account=%u)\n",
                    coin_type, account);
    return true;
}

bool P2TSHScriptPubKeyMan::IsHDEnabled() const
{
    LOCK(cs_p2mr);
    return m_pqhd && m_pqhd->IsSeeded();
}

std::string P2TSHScriptPubKeyMan::GetDescriptorString(const uint256& merkle_root) const
{
    LOCK(cs_p2mr);
    ...
    const P2TSHKeyMetadata& meta = it->second;
...
bool P2TSHScriptPubKeyMan::CanProvide(const CScript& script, SignatureData& sigdata)
{
    LOCK(cs_p2mr);
...
bool P2TSHScriptPubKeyMan::HaveP2MRKeys(const uint256& merkle_root) const
{
...
{
    if (!std::holds_alternative<WitnessV2P2TSH>(dest)) return nullptr;

    const auto& p2mr = std::get<WitnessV2P2TSH>(dest);
    uint256 merkle_root;
    std::copy(p2mr.begin(), p2mr.end(), merkle_root.begin());

    const P2TSHKeyMetadata* meta = GetP2MRMetadata(merkle_root);
...
const P2TSHKeyMetadata* P2TSHScriptPubKeyMan::GetP2MRMetadata(const uint256& merkle_root) const
{
    LOCK(cs_p2mr);
    auto it = mapP2MRMetadata.find(merkle_root);
...
bool P2TSHScriptPubKeyMan::SignP2MRInput(
    ...
    P2TSHSpendType spend_type) const
{
    uint256 merkle_root;
    ...
    const P2TSHKeyMetadata& metadata = meta_it->second;
    const P2TSHLeafInfo& leaf = metadata.GetLeaf(spend_type);
    ...
    int actual_sighash = (spend_type == P2TSHSpendType::HYBRID) ? SIGHASH_ALL : sighash_type;
    ...
    WalletLogPrintf("P2TSH: Signed input %d via %s path\n",
...
bool P2TSHScriptPubKeyMan::Encrypt(const CKeyingMaterial& master_key, WalletBatch* batch) { return false; }
bool P2TSHScriptPubKeyMan::CheckDecryptionKey(const CKeyingMaterial& master_key) { return false; }
...
bool P2TSHScriptPubKeyMan::WriteKey(
    WalletBatch& batch, const uint256& merkle_root, const P2TSHKeyMetadata& metadata)
{
    AssertLockHeld(cs_p2mr);
...
bool P2TSHScriptPubKeyMan::LoadFromDB(WalletBatch& batch)
{
    ...
    auto& database = m_storage.GetDatabase();
    auto db_batch = database.MakeBatch();
    if (!db_batch) return false;

    // Load P2MR metadata
    {
        DataStream prefix;
        prefix << DBKeys::P2MR_METADATA;
        auto cursor = db_batch->GetNewPrefixCursor(prefix);
        if (cursor) {
            ...
            while (cursor->Next(key, value) == DatabaseCursor::Status::MORE) {
                try {
                    ...
                    P2TSHKeyMetadata meta; value >> meta;
    ...
    {
        DataStream prefix;
        prefix << DBKeys::P2MR_SCHNORR_KEY;
        auto cursor = db_batch->GetNewPrefixCursor(prefix);
        if (cursor) {
            DataStream key{}, value{};
            while (cursor->Next(key, value) == DatabaseCursor::Status::MORE) {
    ...
    WalletLogPrintf("P2TSH: Loaded %zu addresses, %zu schnorr, %zu slh-dsa keys\n",
...
std::unordered_set<CScript, SaltedSipHasher> P2TSHScriptPubKeyMan::GetScriptPubKeys() const
{
    ...
    for (const auto& [root, _] : mapP2MRMetadata) {
        scripts.insert(GetScriptForDestination(CTxDestination(WitnessV2P2TSH(root))));
    }
    return scripts;
}
...
int P2TSHScriptPubKeyMan::EstimateWitnessWeight(
    P2TSHSpendType spend_type, const P2TSHKeyMetadata& metadata)
{
    const P2TSHLeafInfo& leaf = metadata.GetLeaf(spend_type);
...
std::string P2TSHScriptPubKeyMan::SpendTypeToString(P2TSHSpendType type)
{
    switch (type) {
        case P2TSHSpendType::SCHNORR:  return "schnorr";
        case P2TSHSpendType::SLH_DSA:  return "slh-dsa";
        case P2TSHSpendType::HYBRID:   return "hybrid";
        default: return "unknown";
    }
}

std::optional<P2TSHSpendType> P2TSHScriptPubKeyMan::StringToSpendType(const std::string& str)
{
    if (str == "schnorr")                    return P2TSHSpendType::SCHNORR;
    if (str == "slh-dsa" || str == "slhdsa") return P2TSHSpendType::SLH_DSA;
    if (str == "hybrid")                     return P2TSHSpendType::HYBRID;
    return std::nullopt;
}
